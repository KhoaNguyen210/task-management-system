...
<head>
    ...
    <title>Chỉnh sửa công việc - Hệ thống quản lý công việc</title>
...
<body class="bg-gray-100 font-sans">
    ...
    <div class="container mx-auto mt-8 px-4 pb-16">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-bold text-gray-800">Chỉnh sửa công việc</h2>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            <h3 class="text-xl font-semibold text-gray-700 mb-4">Chỉnh sửa công việc</h3>
            <form action="{{ route('tasks.update', $task->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $task->title) }}" class="block w-full border-gray-300 rounded-lg shadow-sm p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200" placeholder="Nhập tên công việc" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" rows="4" class="block w-full border-gray-300 rounded-lg shadow-sm p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200" placeholder="Nhập mô tả chi tiết cho công việc" required>{{ old('description', $task->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="assigned_users[]" id="assigned_users" placeholder="Tìm và chọn giảng viên..." @if($task->status === 'In Progress') disabled @else required @endif multiple>
                            @foreach ($lecturers as $lecturer)
                                <option value="{{ $lecturer->user_id }}"
                                    {{ $task->assignedUsers->pluck('user_id')->contains($lecturer->user_id) ? 'selected' : '' }}>
                                    {{ $lecturer->name }} ({{ $lecturer->email }})
                                </option>
                            @endforeach
                        </select>
                        @if($task->status === 'In Progress')
                            <p class="text-sm text-gray-500 mt-1">Không thể thay đổi người thực hiện khi công việc đang thực hiện.</p>
                        @endif
                    </div>

                    <div>
                        <label for="due_date" class="block text-sm font-medium text-gray-700 mb-2">Thời hạn <span class="text-red-500">*</span></label>
                        <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $task->due_date->format('Y-m-d')) }}" class="block w-full border-gray-300 rounded-lg shadow-sm p-3 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200" required min="{{ date('Y-m-d') }}">
                    </div>
                </div>
